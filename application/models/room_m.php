...
class Room_m extends CI_Model
{
    ...
    function getRoomDetail($id){
        return $this->db->where('room_id',$id)->get('room')->row();
    }
    ...
    function getRoomsWithTypes()
    {
        $rooms = $this->db->order_by('room_price', 'asc')->get('room_type')->result();
        foreach ($rooms as $id => $type) {
            $room = $this->db->select('group_concat(room_id) as rooms,room_type')->group_by('room_type')->where('room_type', $type->room_type)->get('room')->row();
            $rooms[$id]
                ->rooms
                = $room;
        }
